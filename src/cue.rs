...
impl<D: ?Sized + DecoderTrait> BlissCue<D> {
    ...
    pub fn songs_from_path<P: AsRef<Path>>(path: P) -> BlissResult<Vec<BlissResult<Song>>> {
        Self::songs_from_path_with_options(path, &AnalysisOptions::default())
    }

    /// Analyze songs from a CUE file, extracting individual [Song] objects
    /// for each individual song, passing options for analysis such as the desired features
    /// version. Use it for backwards compatibility.
    ///
    /// Each returned [Song] has a populated [cue_info](Song::cue_info) object, that can be
    /// be used to retrieve which CUE sheet was used to extract it, as well
    /// as the corresponding audio file.
    pub fn songs_from_path_with_options<P: AsRef<Path>>(
        path: P,
        analysis_options: &AnalysisOptions,
    ) -> BlissResult<Vec<BlissResult<Song>>> {
            ...
            match cue_file {
                Ok(f) => {
                    if !f.sample_array.is_empty() {
                        songs.extend_from_slice(&f.get_songs(analysis_options));
                    } else {
...
impl BlissCueFile {
    ...
    fn get_songs(&self, analysis_options: &AnalysisOptions) -> Vec<BlissResult<Song>> {
        let mut songs = Vec::new();
            ...
            if let Some((_, start_current)) = current_track.indices.first() {
                if let Some((_, end_current)) = next_track.indices.first() {
                    ...
                    let analysis = Song::analyze_with_options(
                        &self.sample_array[start_current..end_current],
                        analysis_options,
                    );
        ...
        if let Some(last_track) = self.tracks.last() {
            if let Some((_, start_current)) = last_track.indices.first() {
                ...
                let analysis = Song::analyze_with_options(
                    &self.sample_array[start_current..],
                    analysis_options,
                );
...
mod tests {
    ...
    #[test]
    #[cfg(feature = "ffmpeg")]
    // Test that analyzing a CUE with old features indeed runs the analysis
    // with old features.
    fn test_cue_analysis_with_options() {
        let songs = BlissCue::<FFmpegDecoder>::songs_from_path_with_options(
            "data/testcue.cue",
            &AnalysisOptions {
        ...
        let expected = vec![
            (
                Path::new("data/testcue.cue/CUE_TRACK001").to_path_buf(),
                Analysis {
                    internal_analysis: vec![
                        0.38463724,
                        -0.85219246,
            ...
            (
                Path::new("data/testcue.cue/CUE_TRACK002").to_path_buf(),
                Analysis {
                    internal_analysis: vec![
                        0.18622077,
                        -0.5989029,
            ...
            (
                Path::new("data/testcue.cue/CUE_TRACK003").to_path_buf(),
                Analysis {
                    internal_analysis: vec![
                        0.0024261475,
                        0.9874661,
        ...
        for ((path, analysis, features_version), song) in expected.into_iter().zip(songs) {
            let song = song.unwrap();
            assert_eq!(path, song.path);
            assert_eq!(analysis, song.analysis);
